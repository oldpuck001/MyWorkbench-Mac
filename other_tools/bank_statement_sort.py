...
def bank_statement_sort_export(request):
    ...
    source_file_path = request.get("data", {}).get("file_path", "")
    ...
    # 建立输出文件夹
    export_path = os.path.join(file_path, f'{file_name}银行对账单分类')
    ...
    for credit_name_sort, credit_name_df in credit_name_dfs.items():
        ...
        for credit_name_number_sort, credit_name_number_df in credit_name_number_dfs.items():
            # 输出单个分类项为xlsx文件
            credit_column_save = sanitize_filename(credit_column)
            credit_name_sort_save = sanitize_filename(credit_name_sort)
            credit_name_number_sort_save = sanitize_filename(credit_name_number_sort)
            # 初始文件名
            base_filename = f'{credit_name_sort_save}_{credit_name_number_sort_save}_{credit_column_save}.xlsx'
            ...
            while os.path.exists(df_export_path):
                ...
                new_filename = f'{credit_name_sort_save}_{credit_name_number_sort_save}_{credit_column_save}_{counter}.xlsx'
                df_export_path = os.path.join(export_path, new_filename)
                counter += 1
            # 保存文件
            credit_name_number_df.to_excel(df_export_path, index=False)
    ...
    for debit_name_sort, debit_name_df in debit_name_dfs.items():
        ...
        for debit_name_number_sort, debit_name_number_df in debit_name_number_dfs.items():
            # 输出单个分类项为xlsx文件
            debit_column_save = sanitize_filename(debit_column)
            debit_name_sort_save = sanitize_filename(debit_name_sort)
            debit_name_number_sort_save = sanitize_filename(debit_name_number_sort)
            # 初始文件名
            base_filename = f'{debit_name_sort_save}_{debit_name_number_sort_save}_{debit_column_save}.xlsx'
            df_export_path = os.path.join(export_path, base_filename)
            # 检查文件是否存在，如果存在则添加后缀
            counter = 1
            while os.path.exists(df_export_path):
                # 如果文件存在，修改文件名，例如：filename_1.xlsx, filename_2.xlsx
                new_filename = f'{debit_name_sort_save}_{debit_name_number_sort_save}_{debit_column_save}_{counter}.xlsx'
                df_export_path = os.path.join(export_path, new_filename)
                counter += 1
            # 保存文件
            debit_name_number_df.to_excel(df_export_path, index=False)
